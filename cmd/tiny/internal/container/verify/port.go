...
func (p *portVerifier) Handle() error {
	...
	case "linux", "darwin":
		if !(p.port >= 1024 && p.port <= 65535) {
			return errors.New(colour.Red(fmt.Sprintf("不可以设置系统预留端口 %d, 您可以设置的范围在 [ 1024 ~ 65535 ] 之间。", p.port)))
		}
	case "windows":
		if !(p.port >= 5001 && p.port <= 65535) {
			return errors.New(colour.Red(fmt.Sprintf("不可以设置系统预留端口 %d, 您可以设置的范围在 [ 5001 ~ 65535 ] 之间。", p.port)))
		}
	}
	return nil
}
