...
func NewClientControl(clientConfig *config.GrpcClientConfig) (*ClientControl, error) {
	...
	mClient := message.NewMessageServiceClient(gClient)
...
func (c *ClientControl) SendMessageBidi(req *message.BaseRequest, chunkSize int) (*message.BaseResponse, error) {
	if chunkSize <= 0 {
