...
type MessageHandler struct {
	handlerMap map[string]func(ctx context.Context, in *message.BaseRequest) (*message.BaseResponse, error)
}

func (h *MessageHandler) RegisterHandler(path string, handle func(ctx context.Context, in *message.BaseRequest) (*message.BaseResponse, error)) {
	h.handlerMap[path] = handle
}

func (h *MessageHandler) GetHandler(path string) (func(ctx context.Context, in *message.BaseRequest) (*message.BaseResponse, error), error) {
...
func (s *server) SendMessageBidi(stream message.MessageService_SendMessageBidiServer) error {
	...
	// 发送 meta（将 handler 返回的 success/code/message 放入 meta）
	_ = stream.Send(&message.Chunk{
		MessageType: req.MessageType,
		ClientId:    req.ClientId,
		Seq:         -1,
	...
	for sent < total {
		...
		err = stream.Send(&message.Chunk{
			MessageType: req.MessageType,
			ClientId:    req.ClientId,
			Seq:         outSeq,
			Last:        end == total,
			Payload: &message.Chunk_Data{
				Data: piece,
			},
		})
		if err != nil {
