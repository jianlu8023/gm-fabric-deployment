...
// @struct
// @property UserID string 用户ID（必填）
type MFARecoverySecretRequest struct {
	UserID string `json:"user_id,omitempty" yaml:"user_id,omitempty" form:"userId" binding:"required,min=1,max=100"` // 用户ID
}
...
// @struct
// @property UserID string 用户ID（必填）
// @property Code string MFA验证码（必填，6位数字）
type MFAVerifyCodeRequest struct {
	UserID string `json:"user_id,omitempty" yaml:"user_id,omitempty" form:"userId" binding:"required,min=1,max=100"` // 用户ID
	Code   string `json:"code,omitempty" yaml:"code,omitempty" form:"code" binding:"required,len=6,numeric"`         // MFA验证码
}
