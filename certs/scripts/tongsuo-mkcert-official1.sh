...
# 使用 tongsuossl ca 命令签发 (CA 层自签除外); serial 由 ca 模式的
# db/<CA名>/serial 自增维护 (x509 -req 的 -CAcreateserial 在 Tongsuo
# 8.4.0 / OpenSSL 3.0.x 下被静默忽略, 故弃用, 详见 ca_sign 注释)
#
# SM2 标识强制 (重要):
...
#   注意: ca 命令的 CSR 自签校验只认默认 ID, SM2_ID 不可改为非默认值。
#
# 一套 CA 两种叶子 (合并生成):
...
gen_intermediate() {
    ...
    ca_sign root.crt root.key intermediate.csr intermediate.crt \
        v3_intermediate_ca "$DAYS_INTERMEDIATE" intermediate.cnf \
        || error "中间证书签发失败"
...
gen_authority() {
    ...
    ca_sign intermediate.crt intermediate.key "$csr_file" "$crt_file" \
        v3_application_ca "$DAYS_AUTHORITY" "$cnf_file" \
        || error "${name}-authority 签发失败"
...
gen_entity_cert() {
    ...
    ca_sign "${authority_name}-authority.crt" "${authority_name}-authority.key" \
        "$csr_file" "$crt_file" "$ext_section" "$DAYS_ENTITY" "$cnf_file" \
        || error "${entity_prefix}_${cert_type} 签发失败"
