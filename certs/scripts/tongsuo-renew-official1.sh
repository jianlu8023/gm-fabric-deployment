...
#          -> 实体证书 (每个实体 3 张: RFC8998 单证书 + GM/T 0024 双证书 sign/enc)
#
# 功能:
...
#
# 国密证书说明:
#   - 每个实体有 3 张证书: RFC8998 单证书 (无后缀, TLS 1.3 单证书模式) +
#     sign (签名) 与 enc (加密) 双证书 (GM/T 0024 NTLS), 私钥相互独立
#   - ./renew.sh renew hserver       -> 只续签 RFC8998 单证书
#   - ./renew.sh renew hserver_sign hserver_enc -> 成对续签双证书
#   - auto 模式下双证书任一张到期即成对续签, 保证 sign/enc 有效期同步
#     (单证书独立续签, 不参与成对逻辑)
#   - 续签使用 SM2 密钥 + SM3 摘要 (-sm3), DN 优先取自旧证书, 缺失时用注册表默认值
#   - 所有 SM2 签名操作显式强制 sm2_id=1234567812345678 (GM/T 标准值,
#     与 mkcert.sh 一致), 保证续签后的链可被 Go gmsm / 原生 OpenSSL 校验
#
# 用法示例:
#   ./renew.sh check                    # 检查全部证书
#   ./renew.sh check hserver            # 只检查 hserver (含单证书与双证书)
#   ./renew.sh renew hserver            # 只续签 RFC8998 单证书 (365 天)
#   ./renew.sh renew hserver_sign hserver_enc -d 730 # 成对续签双证书, 730 天
...
# ============================================================
# ca 模式签发 (替代 x509 -req -CAcreateserial)
# 背景: Tongsuo 8.4.0 / OpenSSL 3.0.x 的 x509 -req 会无条件预填随机
# 序列号, -CAcreateserial/-CAserial 被静默忽略 (OpenSSL 3.5+ 已修复),
# serial 无法管理。改用 ca 命令签发: 每个 CA 一个 db 目录
# (db/<CA名>/{index.txt,serial,newcerts}), serial 由工具自增维护,
# 行为与 rsa 侧 openssl 3.5+ 的 <CA>.srl 等价。
# 注意: ca 的 CSR 自签校验按默认 SM2 ID (1234567812345678) 进行,
# 因此 SM2_ID 必须保持默认值。
# ============================================================
...
ca_db_init() {
    ca_crt="$1"
    ca_key="$2"
    ca_name=$(basename "$ca_crt" .crt)
    ca_dir="$CA_DB_DIR/$ca_name"
    mkdir -p "$ca_dir/newcerts"
    [ -f "$ca_dir/index.txt" ] || : > "$ca_dir/index.txt"
    if [ ! -s "$ca_dir/serial" ]; then
        # 与 OpenSSL -CAcreateserial 行为对齐: 首次随机初始化
        "$TONGSUOSSL" rand -hex 20 > "$ca_dir/serial" 2>/dev/null \
            || od -An -tx1 -N20 /dev/urandom 2>/dev/null | tr -d ' \n' > "$ca_dir/serial"
    fi
    cat > "$ca_dir/ca.cnf" <<EOF
# 由 renew.sh 自动生成 (ca 签发配置)
[ ca ]
...
ca_sign() {
    # 用法: ca_sign <CA证书> <CA私钥> <CSR> <输出crt> <扩展段名> <有效期天> <实体cnf路径>
    # (ca 只认 -config, 无 -extfile, 故把实体 cnf 的扩展段拼进临时配置)
    cs_crt="$1"; cs_key="$2"; cs_csr="$3"; cs_out="$4"; cs_ext="$5"; cs_days="$6"; cs_cnf="$7"

    ca_db_init "$cs_crt" "$cs_key"
    ca_name=$(basename "$cs_crt" .crt)
    ca_dir="$CA_DB_DIR/$ca_name"
    cs_tmpcnf="$ca_dir/.sign.cnf"
    cat "$ca_dir/ca.cnf" "$cs_cnf" > "$cs_tmpcnf"

    "$TONGSUOSSL" ca -config "$cs_tmpcnf" -batch -notext \
        -in "$cs_csr" -out "$cs_out" -extensions "$cs_ext" \
        -days "$cs_days" -md sm3 $SIGOPT_ARGS
    cs_rc=$?
...
hserver|http-authority.crt|http-authority.key|hserver.cnf|server_ext|365|entity|HTTP服务器RFC8998单证书|/C=CN/ST=Xinjiang/L=Urumqi/O=HTTP Certificate Authority/OU=HTTP Certificate/CN=www.jianlu.site
...
hclient|http-authority.crt|http-authority.key|hclient.cnf|client_ext|365|entity|HTTP客户端RFC8998单证书|/C=CN/ST=Xinjiang/L=Urumqi/O=HTTP Certificate Authority/OU=HTTP Certificate/CN=hclient.jianlu.site
...
gserver|grpc-authority.crt|grpc-authority.key|gserver.cnf|server_ext|365|entity|gRPC服务器RFC8998单证书|/C=CN/ST=Xinjiang/L=Urumqi/O=GRPC Certificate Authority/OU=GRPC Certificate/CN=gserver.jianlu.site
...
gclient|grpc-authority.crt|grpc-authority.key|gclient.cnf|client_ext|365|entity|gRPC客户端RFC8998单证书|/C=CN/ST=Xinjiang/L=Urumqi/O=GRPC Certificate Authority/OU=GRPC Certificate/CN=gclient.jianlu.site
...
dserver|docker-authority.crt|docker-authority.key|dserver.cnf|server_ext|365|entity|Docker服务器RFC8998单证书|/C=CN/ST=Xinjiang/L=Urumqi/O=DOCKER Certificate Authority/OU=DOCKER Certificate/CN=dserver.jianlu.site
...
dclient|docker-authority.crt|docker-authority.key|dclient.cnf|client_ext|365|entity|Docker客户端RFC8998单证书|/C=CN/ST=Xinjiang/L=Urumqi/O=DOCKER Certificate Authority/OU=DOCKER Certificate/CN=dclient.jianlu.site
...
aserver|applications-authority.crt|applications-authority.key|aserver.cnf|server_ext|365|entity|Applications服务器RFC8998单证书|/C=CN/ST=Xinjiang/L=Urumqi/O=Applications Certificate Authority/OU=Applications Certificate/CN=aserver.jianlu.site
...
aclient|applications-authority.crt|applications-authority.key|aclient.cnf|client_ext|365|entity|Applications客户端RFC8998单证书|/C=CN/ST=Xinjiang/L=Urumqi/O=Applications Certificate Authority/OU=Applications Certificate/CN=aclient.jianlu.site
...
lserver|libp2p-authority.crt|libp2p-authority.key|lserver.cnf|server_ext|365|entity|libp2p服务器RFC8998单证书|/C=CN/ST=Xinjiang/L=Urumqi/O=LIBP2P Certificate Authority/OU=LIBP2P Certificate/CN=lserver.jianlu.site
...
lclient|libp2p-authority.crt|libp2p-authority.key|lclient.cnf|client_ext|365|entity|libp2p客户端RFC8998单证书|/C=CN/ST=Xinjiang/L=Urumqi/O=LIBP2P Certificate Authority/OU=LIBP2P Certificate/CN=lclient.jianlu.site
...
# 展开用户输入为具体证书名列表 (每行一个):
#   hserver       -> hserver       (RFC8998 单证书, 注册表内直接命中)
...
check_pair_sync() {
    ...
    for cps_b in $cps_bases; do
        ...
        if [ "$cps_diff" -gt "$PAIR_SYNC_TOLERANCE" ]; then
            warn "双证书有效期不同步: ${cps_b}_sign 剩 ${cps_ds} 天, ${cps_b}_enc 剩 ${cps_de} 天 (建议成对续签: ./renew.sh renew ${cps_b}_sign ${cps_b}_enc)"
...
EOF
    ...
    "$TONGSUOSSL" req -config "$ro_cnf" -new -key "$ro_key" -sm3 \
        -out "$ro_csr" -subj "$ro_subj_use" $SIGOPT_ARGS \
    ...
    if [ "$ro_issuer" = "self" ]; then
        ...
        "$TONGSUOSSL" req -new -x509 -key "$ro_key" -sm3 -days "$ro_days" \
            -out "$ro_crt" -config "$ro_cnf" -subj "$ro_subj_use" $SIGOPT_ARGS \
            || { err "  自签名失败"; return 1; }
    else
        info "  由 ${ro_issuer} 签发中..."
        ca_sign "$ro_issuer" "$ro_issuer_key" "$ro_csr" "$ro_crt" \
            "$ro_ext" "$ro_days" "$ro_cnf" \
            || { err "  签发失败"; return 1; }
    fi
    ...
    if [ -n "$ro_pair" ] && [ -f "${ro_pair}.crt" ]; then
        ...
        if [ "$ro_pair_days" != "?" ] && [ $(( ro_new_days - ro_pair_days )) -gt "$PAIR_SYNC_TOLERANCE" ] 2>/dev/null; then
            warn "  双证书不同步: ${ro_pair} 仅剩 ${ro_pair_days} 天, 建议成对续签: ./renew.sh renew ${ro_name%_sign}_sign ${ro_name%_sign}_enc"
        fi
    fi
    return 0
}
...
证书构成:
  每个实体有 3 张证书, 私钥独立:
    hserver        RFC8998 单证书 (GM TLS 1.3, sign+enc 合一)
    hserver_sign   GM/T 0024 签名证书
    hserver_enc    GM/T 0024 加密证书
  ./renew.sh renew hserver                     -> 只续签单证书
  ./renew.sh renew hserver_sign hserver_enc    -> 成对续签双证书
  ./renew.sh check hserver                     -> 只检查 hserver 一张 (用 hserver_sign 检查签名证书)
  auto 模式下双证书任一张到期即成对续签; check 会对不同步的双证书告警。
...
示例:
  ./renew.sh check                    # 检查全部证书
  ./renew.sh check hserver_sign       # 只检查签名证书
  ./renew.sh renew hserver            # 续签 RFC8998 单证书 (默认 365 天)
  ./renew.sh renew hserver_sign hserver_enc -d 730 # 成对续签双证书, 有效期 730 天
  ./renew.sh auto -d 60 -y            # 续签 60 天内到期的证书, 免确认 (适合 cron)
  ./renew.sh verify hserver_sign      # 验证签名证书的链与私钥
  ./renew.sh chains                   # 只重建链文件

说明:
  - 续签保留原有私钥; CA 用同一私钥续签后, 已签发的下级证书仍然有效
  - 签发使用 SM2 密钥 + SM3 摘要; DN 优先继承旧证书, 旧证书缺失时用内置默认值
  - 单证书与双证书的 cnf (hserver.cnf / hserver_sign.cnf / hserver_enc.cnf)
    由 mkcert.sh 统一生成, 续签前请确认已运行过 mkcert.sh
  - 所有 SM2 签名强制 sm2_id=1234567812345678 (GM/T 标准值, 与 mkcert.sh 一致),
    续签后的证书可被 Go gmsm / 原生 OpenSSL 按标准 ID 校验
    (SM2_SIGOPT=0 可关闭强制; 原生 openssl auto 下自动关闭)
