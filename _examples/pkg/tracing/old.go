...
// func myNewTracerProvider(ctx context.Context) (shutdownTracerProvider, error) {
// // OTLP endpoint
// otlpEndpoint := os.Getenv("OTLP_ENDPOINT")
// if otlpEndpoint == "" {
// 	otlpEndpoint = "localhost:4318" // 默认 OTLP 地址
...
// )
//
// exp, err := otlptrace.New(ctx, client) // 使用传入的 context
// if err != nil {
// 	return nil, fmt.Errorf("creating OTLP HTTP exporter: %w", err)
// }
//
...
// // 创建 TracerProvider
// tp := sdktrace.NewTracerProvider(
// 	sdktrace.WithSampler(sdktrace.AlwaysSample()), // 或者自定义采样器
// 	sdktrace.WithBatcher(exp),                     // 使用 OTLP exporter
// 	sdktrace.WithResource(res),                    // 使用 resource
// )
//
// otel.SetTracerProvider(tp)
// otel.SetTextMapPropagator(propagation.NewCompositeTextMapPropagator(propagation.TraceContext{}, propagation.Baggage{}))
