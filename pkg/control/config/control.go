...
func (c *Control) StartUp(failedFunc func(err error)) {
	...
	if c.started.Load() {
		c.mutex.Unlock()
		failedFunc(fmt.Errorf("config server already started, skip startup"))
		return
	}
	...
	if err != nil {
		fmt.Printf("%v  [%v]  config/control.go:62  [config/control]  load config failed: %s\n", time.Now().Format("2006-01-02 15:04:05.000"), colour.Red("ERROR"), err)
		c.mutex.Lock()
...
func (c *Control) printConfig() {
	...
	fmt.Printf("%v  [%v]  config/control.go:32  [config/control]  Config:%v\n", time.Now().Format("2006-01-02 15:04:05.000"), colour.Blue("INFO "), c.config)
}
...
func (c *Control) Flush() error {
	...
	fmt.Printf("%v  [%v]  config/control.go:106  [config/control]  开始热加载配置...\n", time.Now().Format("2006-01-02 15:04:05.000"), colour.Blue("INFO "))
	newConfig, err := c.loadConfig()
	if err != nil {
		fmt.Printf("%v  [%v]  config/control.go:109  [config/control]  热加载配置失败: %v\n", time.Now().Format("2006-01-02 15:04:05.000"), colour.Red("ERROR"), err)
		return fmt.Errorf("热加载配置失败: %w", err)
	}

	// 验证新配置
	if err := c.validateConfig(&newConfig); err != nil {
		fmt.Printf("%v  [%v]  config/control.go:115  [config/control]  新配置验证失败: %v\n", time.Now().Format("2006-01-02 15:04:05.000"), colour.Red("ERROR"), err)
		return fmt.Errorf("新配置验证失败: %w", err)
	}

	oldConfig := c.config
	c.config = &newConfig
	fmt.Printf("%v  [%v]  config/control.go:121  [config/control]  配置热加载成功\n", time.Now().Format("2006-01-02 15:04:05.000"), colour.Blue("INFO "))
	fmt.Printf("%v  [%v]  config/control.go:122  [config/control]  旧配置: %s\n", time.Now().Format("2006-01-02 15:04:05.000"), colour.Blue("INFO "), oldConfig.String())
	fmt.Printf("%v  [%v]  config/control.go:123  [config/control]  新配置: %s\n", time.Now().Format("2006-01-02 15:04:05.000"), colour.Blue("INFO "), newConfig.String())
	return nil
}
...
func (c *Control) WatchDog() {
	fmt.Printf("%v  [%v]  config/control.go:130  [config/control]  watching %s\n", time.Now().Format("2006-01-02 15:04:05.000"), colour.Blue("INFO "), c.flagsControl.GetConfigPath())
}
...
func (c *Control) loadConfig() (Config, error) {
	...
	if !stringer.IsBlank(c.flagsControl.GetConfigType()) {
		...
		err := v.ReadInConfig()
		if err == nil {
			fmt.Printf("%v  [%v]  config/control.go:231  [config/control]  使用配置文件: %s\n", time.Now().Format("2006-01-02 15:04:05.000"), colour.Blue("INFO "), v.ConfigFileUsed())
			if err := v.Unmarshal(&cfg); err != nil {
				return cfg, fmt.Errorf("解析环境特定配置文件失败: %w", err)
			}
			return cfg, nil // 成功读取并解析环境特定配置文件
		} else {
			fmt.Printf("%v  [%v]  config/control.go:237  [config/control]  未找到配置文件: %s, 尝试读取默认配置文件. 错误信息: %v\n", time.Now().Format("2006-01-02 15:04:05.000"), colour.Yellow("WARN "), envSpecificFileName, err)
	...
	fmt.Printf("%v  [%v]  config/control.go:254  [config/control]  使用配置文件: %s\n", time.Now().Format("2006-01-02 15:04:05.000"), colour.Blue("INFO "), v.ConfigFileUsed())
...
// @return *WebSocketConfig WebSocket配置
func (c *Control) GetWebSocketConfig() *WebSocketConfig {
	c.mutex.RLock()
	defer c.mutex.RUnlock()
	return c.config.WebSocketConfig
}
