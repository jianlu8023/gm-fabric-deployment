...
func NewIpfsControl(ipfsConfig *config.IpfsConfig, loggerControl *logger.Control) *Control {
	...
	ipfsLogger := loggerControl.GenLogger("")
	ipfsLogger.Infof("[ipfs/control] starting new IPFS control...")
	...
	ipfsLogger.Infof("[ipfs/control] IPFS control started successfully")
	return ipfs
}
...
func (ipfs *Control) initClient() error {
	ipfs.logger.Debugf("[ipfs/control] initializing IPFS client...")
	...
	if err != nil {
		ipfs.logger.Errorf("[ipfs/control] unable to parse API address: %v", err)
		return err
	}
	ipfs.logger.Debugf("[ipfs/control] connecting to IPFS API at %s", addr)
	// newApi, err := ipfsrpc.NewApi(addr)
	// if err != nil {
	// 	ipfs.logger.Errorf("[ipfs/control] failed to create IPFS client: %v", err)
	// 	return err
	// }
	...
	ipfs.once.Do(func() {
		if ipfs.config.Enabled {
			...
			if err := ipfs.initClient(); err != nil {
				ipfs.logger.Errorf("[ipfs/control] IPFS initialization client failed: %v", err)
				ipfs.cancel()
				if failedFunc != nil {
					failedFunc(err)
				}
			...
			ipfs.logger.Infof("[ipfs/control] starting IPFS service...")

			// 测试连接
			if err := ipfs.testConnection(); err != nil {
				ipfs.logger.Errorf("[ipfs/control] failed to connect to IPFS daemon: %v", err)
				if failedFunc != nil {
					failedFunc(err)
				}
				return
			...
			ipfs.logger.Infof("[ipfs/control] IPFS service started successfully")
		}
...
func (ipfs *Control) Shutdown() error {
	ipfs.logger.Infof("[ipfs/control] shutting down IPFS control...")
	ipfs.cancel()
	// IPFS HTTP客户端不需要显式关闭
	ipfs.logger.Infof("[ipfs/control] IPFS control shutdown successfully")
	return nil
}
...
func (ipfs *Control) testConnection() error {
	ipfs.logger.Debugf("[ipfs/control] testing IPFS connection...")

	// 获取IPFS版本信息来测试连接
	version, commit, err := ipfs.shell.Version()
	if err != nil {
		ipfs.logger.Errorf("[ipfs/control] IPFS version check failed: %v", err)
		return err
	}

	ipfs.logger.Infof("[ipfs/control] connected to IPFS node, version: %s commit: %v", version, commit)
	return nil
}
...
func (ipfs *Control) UploadFile(filePath string) (string, error) {
	ipfs.logger.Infof("[ipfs/control] uploading file to IPFS: %s", filePath)

	// 检查文件是否存在
	fileInfo, err := os.Stat(filePath)
	if err != nil {
		ipfs.logger.Errorf("[ipfs/control] file does not exist: %v", err)
		return "", err
	}

	// 检查是否是目录
	if fileInfo.IsDir() {
		ipfs.logger.Debugf("[ipfs/control] uploading directory: %s", filePath)
		// 对于目录，递归上传
	...
	if err != nil {
		ipfs.logger.Errorf("[ipfs/control] failed to open file: %v", err)
		return "", err
	}
	defer func(file *os.File) {
		if err := file.Close(); err != nil {
			ipfs.logger.Errorf("[ipfs/control] failed to close file: %v", err)
		}
	}(file)
	...
	// )
	// if err != nil {
	// 	ipfs.logger.Errorf("[ipfs/control] failed to upload file: %v", err)
	// 	return "", err
	// }

	// cidStr := immutablePath.String()
	// ipfs.logger.Infof("[ipfs/control] file uploaded successfully, CID: %s", cidStr)
	// return cidStr, nil
	return "", nil
}
...
func (ipfs *Control) uploadDirectory(dirPath string) (string, error) {
	ipfs.logger.Debugf("[ipfs/control] uploading directory recursively: %s", dirPath)
	...
	// 	// 忽略目录本身，只处理其中的文件
	// 	if !info.IsDir() {
	// 		ipfs.logger.Debugf("[ipfs/control] uploading file in directory: %s", path)
	...
	// 	ipfs.logger.Errorf("[ipfs/control] failed to upload directory: %v", err)
	// 	return "", err
	// }
	//
	...
	// // 并返回整个目录的根CID
	// ipfs.logger.Infof("[ipfs/control] directory upload process completed: %s", dirPath)
	//
	// // 这里返回一个临时值，实际实现需要调整
...
func (ipfs *Control) DownloadFile(cidStr string, outputPath string) error {
	ipfs.logger.Infof("[ipfs/control] downloading file from IPFS, CID: %s", cidStr)

	// 解析CID
	// decode, err := cid.Decode(cidStr)
	// if err != nil {
	// 	ipfs.logger.Errorf("[ipfs/control] invalid CID format: %v", err)
	// 	return err
	// }
	...
	if err != nil {
		ipfs.logger.Errorf("[ipfs/control] failed to create output file: %v", err)
		return err
	}
	defer func(outputFile *os.File) {
		err := outputFile.Close()
		if err != nil {
			ipfs.logger.Errorf("[ipfs/control] failed to close output file: %v", err)
		}
	}(outputFile)
	...
	// if err != nil {
	// 	ipfs.logger.Errorf("[ipfs/control] failed to get file from IPFS: %v", err)
	// 	return err
	// }
	...
	// if err != nil {
	// 	ipfs.logger.Errorf("[ipfs/control] failed to write to output file: %v", err)
	// 	return err
	// }

	// ipfs.logger.Infof("[ipfs/control] file downloaded successfully, size: %d bytes, saved to: %s", size, outputPath)
	return nil
}
...
func (ipfs *Control) GetFileContent(cidStr string) ([]byte, error) {
	ipfs.logger.Infof("[ipfs/control] getting file content from IPFS, CID: %s", cidStr)
	...
	// if err != nil {
	// 	ipfs.logger.Errorf("[ipfs/control] failed to read file content: %v", err)
	// 	return nil, err
	// }

	// ipfs.logger.Infof("[ipfs/control] file content retrieved successfully, size: %d bytes", len(content))
	// return content, nil
	return nil, nil
}
